...
Constraints fixed before model evaluation
    ...
    - a candidate is availability-eligible only if it has at least
      ...
      The default is 1 period/year: this is intentionally the minimal
      "GE90 and temporal presence" rule requested for the sequential screen.
...
def main() -> None:
    ...
    if args.n_supports != 10:
        raise ValueError(
            "The V5 design is frozen to exactly 10 supports."
        )
    ...
    if metadata_path.is_file():
        ...
        if metadata.get("selection_complete", False):
            ...
            if actual != expected:
                raise RuntimeError(
                    "A completed V5 selection already exists with different "
                    f"frozen parameters. Existing={actual}; requested={expected}."
                )

            print("=" * 100)
            print("V5 SELECTION ALREADY COMPLETE - REUSING FROZEN DESIGN")
            print("=" * 100)
            print(
                pd.read_csv(
    ...
    print("=" * 100)
    print("V5 CANONICAL WHOLE-BASIN SEQUENTIAL RANDOM GE90 SELECTION")
    print("=" * 100)
