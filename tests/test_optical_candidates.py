...
import et_downscaling.optical_candidates as experiment
...
def load_script(name):
    path = ROOT / "scripts" / "candidates" / name
...
export_script = load_script("export_optical_candidates.py")
build_script = load_script("build_optical_candidates.py")


class OpticalCandidateTests(unittest.TestCase):
    def test_context_and_namespace_are_isolated(self):
        experiment.validate_context("2020-01-01", "2025-01-01", "2020_2024")
        self.assertEqual(export_script.output_root("2020_2024").name, "optical")
