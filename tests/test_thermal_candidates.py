...
import et_downscaling.thermal_candidates as thermal
...
def load_script(filename):
    path = ROOT / "scripts" / "candidates" / filename
...
export_script = load_script("export_thermal_candidates.py")
...
class ThermalConfigurationTests(unittest.TestCase):
    def test_period_and_namespace_are_isolated(self):
        ...
        self.assertEqual(path.name, "landsat_thermal")
    ...
    def test_l8_only_columns_retain_zero_auditable_l9_slot(self):
        ...
        source = inspect.getsource(thermal.build_landsat_thermal)
