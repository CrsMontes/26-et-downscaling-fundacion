"""Production-stack builder for the accepted RF-25 Kc model.

This module builds only the predictors required by RF-25:
...
def build_s2_rf25_predictors(
    ...
    """Build the 16 Sentinel-2 predictors used by RF-25."""
...
def build_rf25_meteorological_predictors(
    period_start: ee.Date,
    period_end: ee.Date,
    number_days: ee.Number,
    processing_geometry: ee.Geometry,
) -> ee.Image:
    """Build the five ERA5-Land fields required by RF-25."""
    era5_source_bands = [
        "temperature_2m",
    ...
    return (
        ...
        .addBands(wind_mean)
        .select(RF25_METEOROLOGICAL_FEATURES)
        .clip(processing_geometry)
        .toFloat()
    )


def build_rf25_production_stack(
    period_start_text: str,
    basin_geometry: ee.Geometry,
) -> dict[str, object]:
    """Build the exact 25-band production stack for RF-25."""
    ...
    optical, optical_period = build_s2_rf25_predictors(
